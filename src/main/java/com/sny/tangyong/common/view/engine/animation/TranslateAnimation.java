...
public class TranslateAnimation extends Animation {
    ...
    protected void applyTransformation(float interpolatedTime, Transformation t) {
        ...
        float dz = mFromZValue;
        
        if (mFromXDelta != mToXValue) {
            dx = mFromXValue + ((mToXValue - mFromXDelta) * interpolatedTime);
        }
        if (mFromYValue != mToYValue) {
            dy = mFromYValue + ((mToYValue - mFromYValue) * interpolatedTime);
        }
        if (mFromZValue != mToZValue) {
            dz = mFromZValue + ((mToZValue - mFromZValue) * interpolatedTime);
        }
